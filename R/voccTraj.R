...
#' @param ang \code{SpatRaster} with velocity angles in degrees.
#' @param mn \code{SpatRaster} with the overall mean climatic value over the period of interest.
...
voccTraj <- function(lonlat, vel, ang, mn, tyr, trajID = 1:nrow(lonlat), correct = FALSE){
  ...
  if(correct == TRUE){
    new_ext_vals <- (rep(terra::res(vel), each = 2)*c(-1,1,-1,1))
    old_ext_vals <- as.vector(terra::ext(vel))
    new_ext_vals <- new_ext_vals + old_ext_vals
    e <- terra::ext(round(new_ext_vals, 2))
...
#TODO: check if below lines can be changed by prod + mask
  vel[is.na(ang)] <- NA
  vel[is.na(mn)] <- NA
  ang[is.na(vel)] <- NA
  mn[is.na(vel)] <- NA
...
# Calculate the trajectories
  ...
  while(sum(remaining <= 0) != nc){  # while there is at least one trajectory active
    utils::setTxtProgressBar(pb, 100*(sum(remaining <= 0)/nc));if (i == 6) break
...
# Step 2. For the rest, get the last point in the starting cell and the first point in a new cell
      ...
      d[d == 0] <- 1
      Trajxy = VoCC:::splitLine(A = llold[resto,], B = lonlat[resto,], n = d)
      ...
      Trajcells <- terra::cellFromXY(vel, data.frame(Trajxy))
      #Trajcells <- apply(Trajxy, 1, function(x){terra::cellFromXY(vel, data.frame(x = x[1], y = x[2]))})
      ...
      remaining[resto][is.na(velend)] <- remaining[resto][is.na(velend)]-((geosphere::distGeo(llold[resto,][is.na(velend),], oldxy[is.na(velend),])/1000)/abs(vel[fcells[resto]][,1][is.na(velend)]))
      remaining[resto][!is.na(velend)] <- remaining[resto][!is.na(velend)]-((geosphere::distGeo(llold[resto,][!is.na(velend),], newxy[!is.na(velend),])/1000)/abs(vel[fcells[resto]][,1][!is.na(velend)]))
...
# Step 3. From step 3 onwards check if the trajectory has bounced back to the origin cell. If so, redirect along cell border.
     if(i >= 1){
      ...
      d[d == 0] <- 1
      Trajxy = VoCC:::splitLine(A = p[rest,], B = destp[rest,], n = d)
      # now get a list with the cells for the points in each trajectory
      if(is.list(Trajxy)){
      Trajcells <- lapply(Trajxy, terra::cellFromXY, object = vel)
      # the first new cell in the trajectory
      ...
      Trajcells <- terra::cellFromXY(vel, as.data.frame(Trajxy))
      ...
      velend[!is.na(velend)][ind][rest] <- terra::extract(vel, terra::cellFromXY(vel, newxy[!is.na(velend),][ind,][rest,]))[,1]
      }}
     }
...
# Step 4. For those traj ending on land redirect the trajectories if possible
     if(sum(is.na(velend)) > 0){
      ...
      Z <- as.numeric(intersect(terra::adjacent(vel, X, directions = 8, pairs = T), terra::adjacent(vel, Y, directions = 8, pairs = T)))
      Z[!is.na(vel[Z])]
      ...
      v <- mapply(function(X,Y) {ifelse(abs(X-Y) > 1, abs(vel[Y][,1])*sin(pi*ang[Y][,1]/180), abs(vel[Y][,1])*cos(pi*ang[Y][,1]/180))}, X = newcell[onland], Y = oldcell[onland])
      ...
      leftcell[k] <- ifelse(vel[focal[k]]>0 & sum(sst[[k]] < as.vector(mn[focal[k]])) > 0, leftcell[[k]][which.min(sst[[k]])],
      ifelse(vel[focal[k]]<0 & sum(sst[[k]] > as.vector(mn[focal[k]])) > 0, leftcell[[k]][which.max(sst[[k]])], NA))
      }
      ...
      loncent$dur <- loncent$dis/abs(unlist(vleft[!is.na(as.numeric(leftcell))]))                 # time taken to get there
     ...
     if (sum(loncent$remaining < 0) > 0) {
       loncent[loncent$remaining < 0, 8:9] <- as.matrix(geosphere::destPoint(
         ...
         (abs(unlist(vleft[!is.na(as.numeric(leftcell))][loncent$remaining < 0])) * (remaining[resto][onland][!empty][!is.na(as.numeric(leftcell))][loncent$remaining < 0]) * 1000)
       ))
